...
value_btc=$(bitcoin-cli -regtest decoderawtransaction "$raw_tx" | jq -r '.vout[0].value')
send_amount_btc=$(jq -n "$value_btc - (20000 / 100000000)")
bitcoin-cli -regtest createrawtransaction '[{"txid":"'"$txid"'","vout":0}]' '{"2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP":'"$send_amount_btc"'}'
