...
# WRITE YOUR SOLUTION BELOW:
UTXO_TXID=$TXID
UTXO_VOUT_INDEX=$(bitcoin-cli -regtest decoderawtransaction "$BASE_TX" | jq -r '.vout | map(select(.value * 100000000 >= 15000000))[0].n')
check_cmd "UTXO vout selection" "UTXO_VOUT_INDEX" "$UTXO_VOUT_INDEX"

UTXO_VALUE=$(bitcoin-cli -regtest decoderawtransaction "$BASE_TX" | jq -r '.vout | map(select(.value * 100000000 >= 15000000))[0].value * 100000000 | round')
...
PAYMENT_BTC=$(jq -n "$PAYMENT_AMOUNT / 100000000 | @text")
CHANGE_BTC=$(jq -n "$CHANGE_AMOUNT / 100000000 | @text")
...
# WRITE YOUR SOLUTION BELOW:
CHANGE_OUTPUT_INDEX=$(bitcoin-cli -regtest decoderawtransaction "$RAW_TX" | jq '.vout | map(select(.scriptPubKey.address == "'"$CHANGE_ADDRESS"'"))[0].n')
...
# Convert to BTC
CHILD_SEND_BTC=$(jq -n "$CHILD_SEND_AMOUNT / 100000000 | @text")
...
# Convert to BTC
TIMELOCK_BTC=$(jq -n "$TIMELOCK_AMOUNT / 100000000 | @text")
